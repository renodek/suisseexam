...
Imported from `renodek/open-design` at `61bda5c`, folder `design-systems/cohere` (`DESIGN.md`, `tokens.css`, `components.html`, `USAGE.md` and the translated `DESIGN-*.md` files). This is an aesthetic inspiration built from public design guidance, not an official brand asset. No font files ship with the source: the type families name the fonts and fall back to system stacks. The component reference is a static page (`components/Reference`), not a live component bundle.
