// Contrôles de livrable/v1-clarte.html : interactions (survol, focus clavier,
// menu mobile, FAQ, formulaire) puis contrastes de tous les couples texte/fond.
// Usage : node scripts/controle-v1.js
...
const OUT = path.join(ROOT, 'mesures', 'v1-controles');
...
async function main() {
  ...
  const V1 = `${base}/livrable/v1-clarte.html?statique`;
  const DESIGN = `${base}/source/design-v1/Accueil%20Clarte.dc.html`;
  ...
  const hoverTargets = [
    ...
    ['Bouton section Solution', '[data-screen-label="Solution"] a:has-text("Réserver mon audit gratuit")'],
    ...
    ['Carte solution', 'article:has-text("Analyse détaillée et complète")'],
    ['Carte bénéfice', 'article:has-text("Gain de temps immédiat")'],
  ...
  report.contrastesSurvol = report.survol.filter((x) => x.v1_survol && /Bouton|Lien/.test(x.element)).map((x) => {
    const fond = x.element.includes('Lien') ? (/pied de page|légal|navigation/.test(x.element) ? { r: 10, g: 14, b: 26 } : (x.element.includes('Découvrir') ? { r: 10, g: 14, b: 26 } : { r: 255, g: 255, b: 255 })) : rgb(x.v1_survol.background);
    return { element: x.element, texte: x.v1_survol.color, fond: 'rgb(' + fond.r + ',' + fond.g + ',' + fond.b + ')', ratio: cr(rgb(x.v1_survol.color), fond) };
  });
  const bordure = { r: 124, g: 134, b: 150 };
  report.composantsInterface = [{ element: 'Bordure des champs du formulaire (#7c8696) sur blanc', ratio: cr(bordure, { r: 255, g: 255, b: 255 }), requis: 3 }, { element: 'Ancienne bordure du design (#9aa4b2) sur blanc', ratio: cr({ r: 154, g: 164, b: 178 }, { r: 255, g: 255, b: 255 }), requis: 3 }, { element: 'Bordure de champ au focus (#0e7490) sur blanc', ratio: cr({ r: 14, g: 116, b: 144 }, { r: 255, g: 255, b: 255 }), requis: 3 }];
