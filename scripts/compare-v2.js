// Compare livrable/v2-nuit-suisse.html au design d'origine (export Claude Design)
// section par section : pixelmatch + écarts de géométrie/style par élément.
// Usage : node scripts/compare-v2.js
...
const OUT_DIR = path.join(ROOT, 'mesures', 'v2-diff');
const VIEWPORTS = [
  { name: '1440', width: 1440, height: 900 },
  { name: '768', width: 768, height: 1024 },
  { name: '390', width: 390, height: 844 },
];
const LABELS = ['header', 'hero', 'chiffres', 'defis', 'solution', 'methode', 'benefices', 'qui', 'temoignages', 'faq', 'contact', 'footer'];
...
async function main() {
  ...
  for (const vp of VIEWPORTS) {
    ...
    for (const [key, url, wait] of [
      ['design', `${base}/source/design-v2/Accueil%20V2%20-%20Nuit%20suisse.dc.html`, true],
      ['page', `${base}/livrable/v2-nuit-suisse.html?statique`, false],
    ]) {
    ...
    for (let i = 0; i < LABELS.length; i++) {
      const d = blocks.design[i], m = blocks.page[i];
      ...
      const b = crop(shots.page, m.top, h, vp.width);
      ...
      report[vp.name][LABELS[i]] = { designHeight: d.height, pageHeight: m.height, mismatchPct: pct, ...cmp };
      console.log(`  ${LABELS[i].padEnd(12)} hauteur design=${d.height} page=${m.height} (Δ${m.height - d.height})  pixelmatch=${pct}%  éléments à écart=${cmp.diffs.length} manquants=${cmp.missing.length} en trop=${cmp.extra.length}`);
    }
  }
  ...
  console.log('\nRapport : mesures/v2-diff/rapport.json');
}
