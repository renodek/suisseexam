...
/* Animations : classe "anim" posée seulement si l'utilisateur n'a pas demandé moins de mouvement, si l'URL ne contient
   ...
   Filet de sécurité : si le script d'animation n'a pas démarré une fois la page chargée (ou après 8 s), la classe est retirée. */
(function(){var d=document.documentElement;if(!window.matchMedia('(prefers-reduced-motion: reduce)').matches&&!/[?&]statique/.test(location.search)&&'IntersectionObserver' in window){d.classList.add('anim');var f=function(){if(!d.hasAttribute('data-anim-ok'))d.classList.remove('anim')};window.addEventListener('load',f);setTimeout(f,8000)}})();
</script>
<!-- ANIM:fin -->
`;
...
const CSS_COMMUN = String.raw`
  ...
  .anim [data-reveal].in{opacity:1;transform:none;transition:opacity .3s var(--ease-out) var(--d,0ms),transform .3s var(--ease-out) var(--d,0ms)}
  ...
  .anim .field-bar{display:block;position:absolute;left:14px;right:14px;bottom:1px;height:2px;border-radius:2px;background:var(--bar-color);transform:scaleX(0);transform-origin:0 50%;pointer-events:none;transition:transform .3s var(--ease-out)}
  .anim .form-group:focus-within .field-bar{transform:scaleX(1)}
  .anim .form-group label{transition:transform .2s var(--ease-out)}
  .anim .form-group:focus-within label{transform:translateX(3px)}
  .anim .btn svg{transition:transform .2s var(--ease-out)}
  .anim .btn:focus-visible svg{transform:translateX(5px)}
  @media (hover:hover) and (pointer:fine){ .anim .btn:hover svg{transform:translateX(5px)} }
  .anim .sc-ring{stroke-dasharray:151;stroke-dashoffset:151;animation:draw .4s var(--ease-out) .05s forwards}
...
const CSS_V1 = String.raw`  :root{--sent-color:#334155;--bar-color:#334155}
  ...
  .anim .methode-step.on .step-circle::before{opacity:.14;transform:scale(1);transition:opacity .3s var(--ease-out),transform .3s var(--ease-out)}
...
const CSS_V2 = String.raw`  :root{--sent-color:var(--g1);--bar-color:var(--g1)}
  ...
  .anim .eyebrow-row.in::before{transform:scaleX(1);transition:transform .6s var(--ease-in-out)}
  .anim .eyebrow-row .num{opacity:0}
  .anim .eyebrow-row.in .num{opacity:1;transition:opacity .3s var(--ease-out) .55s}
  ...
  @keyframes h1-glisse{from{transform:translateY(10px)}}
  .anim h1.hero-title{animation:h1-glisse .4s var(--ease-out) both}
  .anim [data-words]:not(.split){opacity:0}
  .anim .w{display:inline-block;opacity:0;transform:translateY(.3em)}
  .anim .in .w{opacity:1;transform:none;transition:opacity .4s var(--ease-out) calc(var(--i)*45ms),transform .4s var(--ease-out) calc(var(--i)*45ms)}
`;
...
const JS_V2 = String.raw`
    ...
    // Titres H2 : apparition mot par mot (le H1 n'est pas animé en opacité, pour ne pas retarder le LCP). Le texte complet reste dans le HTML ; le découpage se fait ici, à l'exécution,
